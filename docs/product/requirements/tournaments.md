...
Leaderboard visibility is no longer configured per tournament. It is managed as a platform-wide setting on **Administration → General Settings** (`/admin/settings`) and stored in `settings/general.leaderboardVisible`. See platform settings documentation for business rules and contestant experience.
